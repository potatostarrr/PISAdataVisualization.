    ...
    function drawCountry(data, svg, tipPosition) {
        ...
        if (analyze === 0) {
            ...
            var t = svg.append("g")
                ...
                .attr("y", 25)
                .text(function() {
                    if (data[0]["Category"] === "Increase") {
                        return "Countries Female has better Math Performance"
                    } else {
                        return "Countires Male has better Math Performance"
                    }
                })
        }
    ...
    function drawAnalyze(data) {
        ...
        for (j in g) {
            for (i in answer.values()) {
                ...
                decreaseNumber = 0;
                dataIncreased.forEach(function(d) {
                    if (d['Answer'] === answer.values()[i] & d['Gender'] ===
                        g[j]) {
                        increaseNumber += d["number"]
                    }
                });
                dataDecreased.forEach(function(d) {
                    if (d['Answer'] === answer.values()[i] & d['Gender'] ===
                        g[j]) {
                        decreaseNumber += d["number"]
                    }
                });
                l = {
                    Category: "Increase",
